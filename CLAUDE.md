...
**SSL/TLS:** FerretDB is configured to accept only TLS connections. Certs are generated by `setup_certs.sh` and stored in `ferretdb/ssl/`. The backend mounts the rootCA to verify the FerretDB certificate.
